...
- [ ] **EXTRACT-01**: System extracts invoice number with confidence scoring (multiple candidates evaluated, scoring based on position in header, proximity to "Faktura" keywords, uniqueness)
- [ ] **EXTRACT-02**: System extracts total amount with confidence scoring (identifies "Att betala / Total / Summa att betala / Totalt", validates against sum excl + VAT + rounding)
- [ ] **EXTRACT-03**: System extracts vendor name from header
- [ ] **EXTRACT-04**: System extracts invoice date from header
- [ ] **EXTRACT-05**: System stores traceability (page number + bbox + evidence/source text) for invoice number and total (absolute requirement for hard gates)
...
- [ ] **VALID-03**: System assigns confidence scores to all critical fields (invoice number, total)
