...
wave: 2
depends_on: ["01-01"]
files_modified: [src/models/token.py, src/pipeline/tokenizer.py, src/pipeline/pdf_renderer.py, src/pipeline/ocr_abstraction.py, tests/test_tokenizer.py, tests/test_pdf_renderer.py]
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/pipeline/pdf_renderer.py"
      provides: "PDF page to image conversion with standardized DPI (300) and consistent coordinates"
  ...
  key_links:
    ...
    - from: "src/pipeline/pdf_renderer.py"
      to: "src/models/page.py"
      via: "Sets Page.rendered_image_path after rendering"
      pattern: "rendered_image_path|page\\.rendered_image_path"
    - from: "src/pipeline/ocr_abstraction.py"
      to: "src/pipeline/pdf_renderer.py"
      via: "Uses rendered_image_path from Page for OCR"
      pattern: "render_page_to_image|rendered_image_path"
    - from: "src/pipeline/ocr_abstraction.py"
      to: "src/models/page.py"
...
<task type="auto">
  <name>Task 3: Implement PDF page to image rendering</name>
  <files>src/pipeline/pdf_renderer.py</files>
  <action>
Implement PDF page to image conversion with standardized DPI (300) and consistent coordinate system.

Function to implement:
- `render_page_to_image(page: Page, output_dir: str) -> str`: Converts PDF page to image, saves to file, returns path.

Requirements (DOC-03):
- Use pymupdf (fitz) or pdf2image (Poppler) for rendering
- Standardized DPI: 300 DPI (consistent across all renders)
- Consistent coordinate system: Ensure rendered image coordinates match Page coordinate system
- Save rendered image to output_dir, filename: `{document_filename}_page_{page_number}.png`
- Set Page.rendered_image_path to saved image path
- Handle multi-page documents: render each page separately

Image requirements:
- Format: PNG (lossless, suitable for OCR)
- DPI: 300 (standardized, high quality for OCR)
- Coordinate system: Match Page coordinate system (top-left origin, Y increases downward)

If using pdf2image:
- Requires Poppler system dependency
- Use `pdf2image.convert_from_path` with dpi=300
- Save to output_dir

If using pymupdf (fitz):
- Use `fitz.open` and `page.get_pixmap(dpi=300)`
- Save pixmap to PNG file
- No system dependencies needed

Choose one: pymupdf recommended (no system dependencies), pdf2image alternative.

Error handling: If rendering fails (corrupt page, missing dependencies), raise exception with clear error message.
  </action>
  <verify>python -c "from src.pipeline.reader import read_pdf; from src.pipeline.pdf_renderer import render_page_to_image; import tempfile; doc = read_pdf('tests/fixtures/pdfs/sample_invoice_1.pdf'); page = doc.pages[0]; with tempfile.TemporaryDirectory() as tmpdir: img_path = render_page_to_image(page, tmpdir); assert page.rendered_image_path == img_path; import os; assert os.path.exists(img_path)"</verify>
  <done>render_page_to_image converts PDF page to 300 DPI image, saves to file, sets Page.rendered_image_path, coordinate system matches Page</done>
</task>

<task type="auto">
  <name>Task 4: Implement OCR abstraction layer with Tesseract</name>
...
- Require Page.rendered_image_path to exist (from pdf_renderer)
...
- Convert Tesseract coordinates to Page coordinate system (standardize to match pdfplumber coordinate system, accounting for DPI scaling)
...
Error handling: If Tesseract not installed or Swedish language data missing, raise clear error with installation instructions. If OCR fails or rendered_image_path missing, raise exception (caller handles).
  </action>
  ...
  <done>OCR abstraction layer implemented, TesseractOCREngine extracts tokens with bbox + confidence from TSV/HOCR, requires Page.rendered_image_path, interface allows future engine switching</done>
</task>

<task type="auto">
  <name>Task 5: Write unit tests for token extraction and PDF rendering</name>
  <files>tests/test_tokenizer.py, tests/test_pdf_renderer.py</files>
  <files>tests/test_tokenizer.py</files>
  <action>
Create unit tests for token extraction, PDF rendering, and OCR abstraction.
...
- test_pdf_renderer.py:
  - Test PDF page to image rendering (300 DPI)
  - Test Page.rendered_image_path is set correctly
  - Test image file exists and is valid PNG
...
- PDF pages rendered to images with correct DPI
- rendered_image_path set correctly

If test corpus has scanned PDF, test OCR path. Otherwise, mock OCR for testing abstraction layer interface.

Use pytest fixtures. Ensure tests run with `pytest tests/test_tokenizer.py tests/test_pdf_renderer.py -v`.
  </action>
  <verify>pytest tests/test_tokenizer.py tests/test_pdf_renderer.py -v</verify>
  <done>All token extraction and PDF rendering unit tests pass, coverage >80% for tokenizer, pdf_renderer, and ocr_abstraction modules</done>
</task>
...
- [ ] pytest tests/test_tokenizer.py tests/test_pdf_renderer.py -v passes
- [ ] mypy type checking passes for all new files
- [ ] Tokens extracted with valid bbox (x, y, width, height) from pdfplumber
- [ ] PDF pages rendered to images with 300 DPI, Page.rendered_image_path set
...
- PDF renderer converts pages to 300 DPI images with consistent coordinates (DOC-03)
